...
def _parse_connect_dsn_and_args(
    *,
    ...
    if user is None:
        raise exceptions.InterfaceError("could not determine user name to connect with")
    ...
    if not addrs:
        raise ValueError("could not determine the database address to connect to")
...
async def _connect_addr(*, addr, loop, timeout, params, config, connection_class):
    ...
    proto_factory = lambda: protocol.Protocol(addr, connected, params, loop)
